...
public class ChannelViewPresenter {
    ...
    public ChannelViewPresenter(final Channel channel, final ChannelView view) {
        ...
        channel.addIRCEventListener(this.formatter);

        // TODO show join/part/quit/nick
        channel.addIRCEventListener(new IRCEventAdapter() {
            @Override
            public void onChannelMessage(MessageEvent e) {
                if (e.getChannel().getName().equals(channel.getName())) {
                    handleMessage(e.getNick(), e.getMessage());
                }
            }
        });
